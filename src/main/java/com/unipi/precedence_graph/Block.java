        ...
        if(independentProcess!=null){
            ...
            Predicate<Process> ascendantProcessesCompleted = new Predicate<Process>() {
                ...
                public boolean test(Process process) {
                    ...
                    List<String> ascendantsName = process.getDependencies(); // βρίσκω τα ονοματα των εργασιών που
                    // πρέπει να εχουν ολοκληρωθεί
                    for(String processName:ascendantsName){ // για καθε μία από αυτές βρίσκω με βάση το όνομα το Οbject process
                        Predicate<Process> processNameFilter = processFilter -> process.getName().equals(processName);
